...
const mip = {
    ...
    // 2026-05-02 (pass-5) - secondary "Find another match" CTA below the
    // helper line. Active at n=1 and n=2 to give the bottom of the page
    // an exit action when the row stack doesn't fill the void. Ghost-tier
    // so it stays quieter than per-card Resume buttons.
    SECONDARY_FIND_Y: -460,
...
const LayoutSpec = {
    ...
    MatchesInProgressPanel: {
        ...
        elements: {
            ...
            // 2026-05-02 (pass-5) - ghost-tier secondary CTA below helper
            // line. Visible at n=1 and n=2; routes to FindMatchPanel.
            secondaryFind: { x: 0, y: mip.SECONDARY_FIND_Y, w: 200, h: 40, type: 'btnGhost' },
    ...
    TokenDuelPanel: {
        ...
        elements: {
            ...
            // and competed with the YOUR SQUAD section for hierarchy.
            feedColumnHeaders:  { x: -2000, y: -2000, w: 1, h: 1, type: 'group',
                notes: '2026-05-02 - moved off-canvas; 3-zone rows obviate the table header strip.' },
            ...
            wagerStartButton:   { x:    0, y: -554, w: UNIFORM_LAYOUT.CONTENT_W, h: 88, type: 'btnPrimary',
                notes: '2026-05-02 polish - moved -444 → -554 (below wager pills). 30 px gap from wager-pill bottom (-480) to CTA top (-510); 30 px gap from CTA bottom (-598) to hint top (-628). 2026-05-02 UX-2 - w 640 → CONTENT_W(680) for full-width hero CTA.' },
            ...
            wagerDropdown:      { x: +130, y: -282, w: 360, h: 402, type: 'group',
                notes: '2026-05-02 polish - shifted -380 → -282 to follow wagerValueButton up (delta 98 px = button delta -542→-444). Anchored above the wager amount pill. 2026-05-03 - height bumped 360 → 402 to fit 9 rows (8 SOL tiers + 1 hidden / 9 SKR tiers including new 2K/3K/4K). AppUI._pinDropdownAboveButton overrides y at runtime so the popup hugs the trigger button.' },
        ...
        templates: {
            ...
            // 8c - 9 wager dropdown rows; hidden until WagerValueButton tapped.
            // Display order is DESCENDING $$ (smallest at bottom, per user pref).
            // Row 0 = largest, row N = smallest. INTRO (SOL) lands on the very
            // bottom row naturally and gets gold-tint sprite + label color at
            // scene-gen. SOL uses 8 rows (row 8 hidden via UIOpacity); SKR uses
            // all 9. Final label/order is set at runtime by AppUI via
            // WAGER_DISPLAY_TO_TIER + WAGER_DISPLAY_TO_TIER_SKR; the literal
            // labels here are just the scene-gen placeholders for the SOL set.
            wagerDropdownRow: {
                count: 9, rowH: 42, padding: 12, w: 340, h: 38,
                labels: ['5 SOL',   '1 SOL',    '0.5 SOL', '0.25 SOL',
                         '0.1 SOL', '0.05 SOL', '0.01 SOL',
                         '0.001 · INTRO', ''],
            },
    ...
    PortfolioPanel: {
        ...
        templates: {
            ...
            heroPnLCard: {
                ...
                // SOL row was y:6 h:64; shrunk to h:44 and lifted to y:18 so the
                // SKR row fits under it on the Real subtab without restyling
                // the whole card. Paper subtab hides ValueSkr at runtime so the
                // SOL row still reads as the dominant value.
                value:    { x: 0, y: 22,  w: 580, h: 44 },
                valueSkr: { x: 0, y: -22, w: 580, h: 44 },
                subtitle: { x: 0, y: -64, w: 580, h: 18 },
            },
